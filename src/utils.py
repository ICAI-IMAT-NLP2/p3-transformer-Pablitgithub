...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([
            AttentionHead(d_model=d_model, d_k=d_k, d_q=d_k, d_v=d_v) for _ in range(num_attention_heads)
        ])
    ...
    def forward(self, x_q, x_k, x_v, mask=None):
        ...
        x = torch.cat([head(x_q, x_k, x_v, mask=mask) for head in self.heads], dim=-1)
