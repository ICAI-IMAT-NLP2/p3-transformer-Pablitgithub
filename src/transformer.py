...
class Transformer(nn.Module):
    ...
    def __greedy_decode(self, src_input: torch.Tensor, max_length: int, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            dec_output = self.decoder(tgt_input, enc_output)
            logits = self.output_linear(dec_output)  # (B, seq_len, vocab_size)
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                for next_token in range(log_probs.size(1)):
                    new_seq = torch.cat([seq, torch.tensor([[next_token]], dtype=torch.long, device=device)], dim=1)
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            next_token_rel = torch.multinomial(probs, num_samples=1)  # Shape: (batch_size, 1)
            next_token = topk_indices.gather(-1, next_token_rel)
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            # Pass through the decoder to get current context representation
            dec_output = self.decoder(tgt_input, enc_output)
            # Project to vocabulary size
            ...
            expanded_tgt_input = tgt_input.repeat_interleave(k, dim=0)  # (batch_size*k, seq_len)
            next_tokens = topk_indices.view(-1, 1)  # (batch_size*k, 1)
            y_candidates = torch.cat([expanded_tgt_input, next_tokens], dim=1)  # (batch_size*k, seq_len+1)

            # Pass each candidate through the decoder to obtain hidden states
            dec_outputs_candidate = self.decoder(y_candidates, enc_output.repeat_interleave(k, dim=0))

            # Extract hidden states
            h_v = dec_outputs_candidate[:, -1, :]  # (batch_size*k, hidden_size)
            h_j = dec_outputs_candidate[:, :-1, :]  # (batch_size*k, seq_len, hidden_size)
            ...
            cos_sim = torch.bmm(h_j_norm, h_v_norm.unsqueeze(-1)).squeeze(-1)  # (batch_size*k, seq_len)

            # Get maximum cosine similarity for each candidate
            max_sim = cos_sim.max(dim=-1)[0]  # (batch_size*k,)

            # Compute scores
            P_LM_v = topk_probs.view(-1)  # flatten probabilities (batch_size*k,)
            scores = (1 - alpha) * P_LM_v - alpha * max_sim

            # Select the candidate with the highest score per batch
            scores = scores.view(batch_size, k)
            best_idx = torch.argmax(scores, dim=-1)  # (batch_size,)
            best_token = topk_indices.gather(1, best_idx.unsqueeze(-1))  # (batch_size, 1)
